...
import { askChatbot, type ChatbotInput } from '@/ai/flows/chatbot';
...
export async function generateChatResponse(input: ChatbotInput): Promise<{ success: boolean; response?: string; error?: string }> {
    try {
        ...
        console.error(e);
        const errorMessage = e instanceof Error ? e.message : 'An unknown error occurred.';
        return { success: false, error: `Failed to get response from chatbot: ${errorMessage}.` };
    }
}
